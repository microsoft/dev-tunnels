...
public interface ITunnelClient : IAsyncDisposable
{
    ...
    Task ConnectAsync(TunnelV2 tunnel, CancellationToken cancellation)
    ...
    /// <exception cref="TunnelConnectionException">The client failed to connect to the
    /// tunnel, or connected but encountered a protocol errror.</exception>
    Task ConnectAsync(TunnelV2 tunnel, string? hostId, CancellationToken cancellation);
    ...
    /// </summary>
    /// <remarks>
    /// Set <see cref="AcceptLocalConnectionsForForwardedPorts"/> to <c>false</c> before calling <see cref="ConnectAsync(TunnelV2, CancellationToken)"/> to ensure
